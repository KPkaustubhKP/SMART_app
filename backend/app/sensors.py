...
class SensorManager:
    ...
    def _generate_realistic_reading(self):
        ...
        readings['soil_moisture'] = max(10, min(90,
            self.base_values['soil_moisture'] + moisture_variation + random.gauss(0, 2)
        ))

        readings['soil_temperature'] = max(5, min(45,
            self.base_values['soil_temperature'] + temp_variation * 0.3 + random.gauss(0, 0.5)
        ))
        ...
        readings['air_temperature'] = max(-10, min(50,
            self.base_values['air_temperature'] + temp_variation + random.gauss(0, 1)
        ))

        readings['humidity'] = max(20, min(95,
            self.base_values['humidity'] + humidity_variation + random.gauss(0, 3)
        ))
        ...
        npk_readings = NPKReading(
            nitrogen=max(50, min(250, self.base_values['nitrogen'] + random.gauss(0, 2))),
            phosphorus=max(20, min(80, self.base_values['phosphorus'] + random.gauss(0, 1))),
            potassium=max(100, min(300, self.base_values['potassium'] + random.gauss(0, 3))),
            timestamp=now
        )
